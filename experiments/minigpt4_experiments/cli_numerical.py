...
model_name = 'minigpt4'
datasets = ['numerical-r' + str(r) for r in range(4)]

# 提问
prefixs =  [""]

questions = ['What is the answer to the arithmetic question in the image?\nOptions: (A) {} (B) {}\nThe answer is']
        ...
        for dataset in datasets:
            with open(os.path.join(log_root, dataset + '-' + model_name + '-' + prompt), 'w') as f:
                ...
                for img in images:
                    ...
                    label = img.split('.')[-2].split('-')[-2]
                    mislabel = img.split('.')[-2].split('-')[-1]
